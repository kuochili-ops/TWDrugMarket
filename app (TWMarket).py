...
if st.checkbox("顯示第四層 ATC 類別金額佔比", value=False):
    ...
    if not sub_df_product.empty:
        ...
        ratio_df = pd.DataFrame({
            ...
            '第四層總金額': [total_22, total_23, total_24],
            '佔比(%)': [
                (current_22 / total_22 * 100) if total_22 else 0,
                (current_23 / total_23 * 100) if total_23 else 0,
                (current_24 / total_24 * 100) if total_24 else 0
            ]
        })
